...
zero_distance_threshold = 0
number_of_features = 65

types = np.genfromtxt("data/classes.csv", dtype=str, delimiter=",")
...
selected_types = np.array([
    'bottle','body','cap','pottedplant','plant','pot',
    'tvmonitor','screen','chair','sofa','diningtable'
], dtype=str)
...
print("Sample types from dataset:", types[:10])
...
def containment_ratios_between_two_bbxes(bb1, bb2):
    ...
    return [
        float(bb_area_intersection) / (bb1_area + 1e-8),
        float(bb_area_intersection) / (bb2_area + 1e-8)
    ]
...
def get_data(train_or_test_swritch,max_rows=10000000):
    ...
    idx_of_cleaned_data = np.arange(len(data))
