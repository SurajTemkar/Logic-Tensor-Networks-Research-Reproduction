...
number_of_positive_examples_x_types=50
number_of_negative_examples_x_types=50
number_of_positive_example_x_partof=50
number_of_negative_example_x_partof=50
number_of_pairs_for_axioms = 500
...
existing_types = [t for t in selected_types if idxs_of_positive_examples_of_types[t].size > 50]
...
for t in existing_types:
    if idxs_of_positive_examples_of_types[t].size > 0:
        clauses_for_positive_examples_of_types.append(
            ltn.Clause(
                [ltn.Literal(True, isOfType[t], objects_of_type[t])],
                label="examples_of_" + t,
                weight=1.0
            )
        )
...
for t in existing_types:
    if idxs_of_negative_examples_of_types[t].size > 0:
        clauses_for_negative_examples_of_types.append(
            ltn.Clause(
                [ltn.Literal(False, isOfType[t], objects_of_type_not[t])],
                label="examples_of_not_" + t,
                weight=1.0
            )
        )
...
clauses_for_parts_of_wholes = [ltn.Clause([ltn.Literal(False, isOfType[w], w1[w]),
                                          ...
                                          label = "parts_of_" + w) for w in list(parts_of_whole.keys())]
...
clauses_for_wholes_of_parts = [ltn.Clause([ltn.Literal(False, isOfType[p], p2[p]),
                                          ...
                                          label="wholes_of_" + p) for p in list(wholes_of_part.keys())]
...
def add_noise_to_data(noise_ratio):
    if noise_ratio > 0:
        ...
        noisy_data_idxs = np.random.choice(list(range(len(train_data))), int(len(train_data) * noise_ratio),replace=False)
...
def get_feed_dict(idxs_of_pos_ex_of_types,
    ...
    # -----------------------------
    # TYPE DATA (INSIDE LOOP)
    # -----------------------------
    for t in existing_types:

        # SAFE positive sampling
        pos_idx = idxs_of_pos_ex_of_types[t]
        pos_sample_size = min(len(pos_idx), number_of_positive_examples_x_types)

        feed_dict[objects_of_type[t].tensor] = \
            train_data[np.random.choice(pos_idx, pos_sample_size, replace=True)][:,1:]

        # SAFE negative sampling
        neg_idx = idxs_of_neg_ex_of_types[t]
        neg_sample_size = min(len(neg_idx), number_of_negative_examples_x_types)

        feed_dict[objects_of_type_not[t].tensor] = \
            train_data[np.random.choice(neg_idx, neg_sample_size, replace=True)][:,1:]

    # -----------------------------
    # PART-OF DATA (OUTSIDE LOOP)
    # -----------------------------
    pos_idx = idxs_of_pos_ex_of_partOf
    size = min(len(pos_idx), number_of_positive_example_x_partof)

    feed_dict[object_pairs_in_partOf.tensor] = \
        pairs_of_train_data[np.random.choice(pos_idx, size, replace=True)]

    neg_idx = idxs_of_neg_ex_of_partOf
    size = min(len(neg_idx), number_of_negative_example_x_partof)

    feed_dict[object_pairs_not_in_partOf.tensor] = \
        pairs_of_train_data[np.random.choice(neg_idx, size, replace=True)]

    # -----------------------------
    # AXIOMS DATA
    # -----------------------------
    tmp = pairs_data[np.random.choice(
        list(range(pairs_data.shape[0])),
        number_of_pairs_for_axioms
    )]
    ...
    if with_constraints:
        for t in selected_types:
            ...
            feed_dict[w1[t].tensor] = \
                tmp[:, number_of_features-1:2*(number_of_features-1)]
            feed_dict[p1[t].tensor] = \
                tmp[:, 0:number_of_features - 1]
        ...
        feed_dict[oo.tensor] = np.concatenate([
            tmp[:, :number_of_features-1],
            tmp[:, :number_of_features-1],
            np.ones((tmp.shape[0], 2), dtype=float)
        ], axis=1)
